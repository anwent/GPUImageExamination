...
@interface ZGPUImageMovieWriter ()
{
...
@implementation ZGPUImageMovieWriter
...
@synthesize assetWriterPixelBufferInput = _assetWriterPixelBufferInput;
...
- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize fileType:(NSString *)newFileType outputSettings:(NSMutableDictionary *)outputSettings;
{
    ...
    NSURL *test_movieURL = [[NSBundle mainBundle] URLForResource:@"abc" withExtension:@"mp4"];
    _th_movie = [[THImageMovie alloc] initWithURL:test_movieURL];
...
// 如果资产读取器和写入器都启动成功，则创建将进行重新编码的调度组并开始一个样本写入会话。
...
//    [self kickOffVideoWriting];
    [self enableSynchronizationCallbacks];
...
- (void)kickOffAudioWriting {
    ...
    // 获取到视频中最短的时间, 只有一个视频，所以直接获取
    AVAsset *asset = _th_movie.asset;
    shortestDuration = asset.duration;
    
    NSLog(@"---视频时间: %.2f", CMTimeGetSeconds(shortestDuration));
    ...
    [assetWriterAudioInput requestMediaDataWhenReadyOnQueue:[THImageMovieManager shared].rwAudioSerializationQueue usingBlock:^{
        ...
        while ([assetWriterAudioInput isReadyForMoreMediaData] && !completedOrFailed) {
            ...
            if (sampleBuffer != NULL) {
                CMTime currentSampleTime = CMSampleBufferGetOutputDecodeTimeStamp(sampleBuffer);
...
- (void)kickOffVideoWriting {
    ...
    [assetWriterVideoInput requestMediaDataWhenReadyOnQueue:[THImageMovieManager shared].rwAudioSerializationQueue usingBlock:^{
        ...
        while ([assetWriterVideoInput isReadyForMoreMediaData] && !completedOrFailed) {
            if (!self.isFrameRecieved) {
                ...
                // line: 484
                BOOL hasMoreFrame = [_th_movie renderNextFrame];
                NSLog(@"--movie: %@, has more frames: %d", _th_movie.url.lastPathComponent, hasMoreFrame);
                if (!hasMoreFrame) {
                    completedOrFailed = YES;
                    break;
                }
